...
void LevelSelection::update(const float& dt)
{
	...
	if (active_layer == DIFFICULTY_SELECTION) {
		...
		for (auto& b : m_Levels) if (b->m_PlayButton->was_pressed()) {
			std::string type;
			if (active_layer == EASY_LEVELS)		 type = "easy";
			else if (active_layer == HARD_LEVELS)	 type = "hard";
			else if (active_layer == EXPERT_LEVELS)  type = "expert";
			std::string level_path = "levels/" + type + "/";
			level_path += type + "_" + std::to_string(b->m_Number) + ".lvl";
			StatesManager::get().create_active_state(new GamePlay(level_path));
		}
...
LevelSelection::LevelSelection()
{
	...
	m_EditorButton->assign_sprite("editor-btn", "editor-btn-pressed");

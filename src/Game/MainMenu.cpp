...
MainMenu::MainMenu()
{
	...
	m_PlayButton = new UIButton("GRAJ", { 3.f, 3.f }, btn_font_size);
	m_PlayButton->set_position({ 0.f, .4f }).center_x();
	make_entity(m_PlayButton);

	m_OptionsButton = new UIButton("OPCJE", { 3.f, 3.f }, btn_font_size);
	m_OptionsButton->set_position({ 0.f, .55f }).center_x();
	make_entity(m_OptionsButton);

	m_ExitButton = new UIButton(L"WYJD", { 3.f, 3.f }, btn_font_size);

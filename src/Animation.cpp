...
bool Animation::play_animation(const std::string& name, uint8_t fps,
                               uint8_t mode, uint8_t repeat_from)
{
...
bool Animation::new_animation(const std::string& name, uint16_t x, uint16_t y,
                              uint16_t w, uint16_t h, uint8_t frames)
{
    if (m_SpriteSheet != nullptr) {
        m_Animations[name] = std::vector<sf::Sprite*>();
        for (uint16_t i = x; i < x + w * frames; i += w) {
            vec2f scale = { m_AnimationOwner->m_Scale.x * WindowHandle::get_ratio().x,
                            m_AnimationOwner->m_Scale.y * WindowHandle::get_ratio().y };
